    ...
    switch (role) {
        case 'parent':
            return Parent.findOne({ studentId: identity, isDeleted: false })
                .then(student => {
                    if (student && passwordHash.verify(password, student.password)) {
                        const data = {
                        ...
                        if (!student.refreshToken) {
                            ...
                            try {
                                jwt.verify(student.refreshToken, process.env.REFRESH_TOKEN_SECRET_KEY)
        ...
        case 'teacher':
            return Teacher.findOne({ email: identity, isDeleted: false })
                .then(teacher => {
                    if (teacher && passwordHash.verify(password, teacher.password)) {
                        const data = {
                            name: teacher.name,
                            email: teacher.email,
                        ...
                        if (!teacher.refreshToken) {
                            ...
                            try {
                                jwt.verify(teacher.refreshToken, process.env.REFRESH_TOKEN_SECRET_KEY)
        ...
        case 'admin':
            return Admin.findOne({ email: identity, isDeleted: false })
                .then(admin => {
                    if (admin && passwordHash.verify(password, admin.password)) {
                        const data = {
                            email: admin.email,
                            role: 'admin'
                        }
                        ...
                        if (!admin.refreshToken) {
                            ...
                            try {
                                jwt.verify(admin.refreshToken, process.env.REFRESH_TOKEN_SECRET_KEY)
